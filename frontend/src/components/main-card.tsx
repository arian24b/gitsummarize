...
export default function MainCard({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const targetPath = `/${sanitizedUsername}/${sanitizedRepo}`;
    
    // Use handleNavigation if available, otherwise directly navigate
    if (handleNavigation) {
      // Create a synthetic mouse event
      const syntheticEvent = {
        // This empty method is intentional as we're creating a minimal synthetic event
        preventDefault: () => { /* No-op implementation for synthetic event */ },
      } as React.MouseEvent<HTMLAnchorElement>;
      
      handleNavigation(syntheticEvent, targetPath);
    } else {
  ...
  return (
    <Card className="relative w-full max-w-3xl border-[3px] border-black bg-green-200 p-4 shadow-[8px_8px_0_0_#000000] sm:p-8">
                ...
                <div className="flex flex-col items-center gap-4 sm:flex-row sm:gap-4">
                  {showCustomization &&
                    ...
                    lastGenerated && (
                      ...
                      >
                        <span>Customize Documentation</span>
                        {activeDropdown === "customize" ? (
                          <ChevronUp size={20} />
                        ) : (
                          <ChevronDown size={20} />
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(exampleRepos).map(([name, path]) => (
                <Button
                  key={name}
                  variant="outline"
                  className="border-2 border-black bg-green-400 text-sm text-black transition-transform hover:-translate-y-0.5 hover:transform hover:bg-green-300 sm:text-base"
      ...
      <div className="absolute -bottom-8 -left-12 hidden sm:block">
        <Sparkles
          className="h-20 w-20 fill-amber-400 text-black"
          strokeWidth={0.6}
